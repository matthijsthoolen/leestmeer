...
function updatePad(data) {
	...
	content.children('div').each(function () {		
		...
		//$(this).html('<span style="background-color: #111">' + content + '</span>');
		...
		if (content === '') {
			...
			if (data.text.hasOwnProperty("i")) {
				color = data.text[i].paragraphColour;
			}
		} else {
			console.log(color);
			var style = {};
			...
			if (first) {
				style['border-top'] = '1px dotted ' + '#123456';
			...
			if (last) {
				style['border-bottom'] = '1px dotted ' + '#123456';
