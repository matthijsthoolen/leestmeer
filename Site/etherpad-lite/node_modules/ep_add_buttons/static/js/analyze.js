...
function addHintsToSidebar(data) {	
	...
	//readable format!
	for (var k in config.checks) {
		cC = config.checks[k];
		
		if (cC.combine && cC.warnings > 0) {
			text += cC.message.head + '(' + cC.warnings + ')<br>';
		}
	}
...
function addFeedbackColumn(config) {
	...
	for (var i in config.defaults.headers) {
		...
		content = content +
					'<dt>' +
						'<a href="#' + importance.key + '" aria-expanded="false" aria-controls="accordion1" class="accordion-title accordionTitle js-accordionTrigger">' + 
							importance.title +
						'</a>' + 
					'</dt>' +
					'<dd class="accordion-content accordionItem is-collapsed" id="' + importance.key + '" aria-hidden="true">' +
